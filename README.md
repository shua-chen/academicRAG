...
* How to start? Ref to: [examples/AcademicRAG_zhipu_postgres_demo.py](https://github.com/HKUDS/AcademicRAG/blob/main/examples/AcademicRAG_zhipu_postgres_demo.py)
...
### Generate Query
AcademicRAG uses the following prompt to generate high-level queries, with the corresponding code in `example/generate_query.py`.
...
### Batch Eval
To evaluate the performance of two RAG systems on high-level queries, AcademicRAG uses the following prompt, with the specific code available in `example/batch_eval.py`.
...
For the extracted contexts, we insert them into the AcademicRAG system.
...
### Step-3 Query
For the queries generated in Step-2, we will extract them and query AcademicRAG.

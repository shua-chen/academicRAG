"""
This module contains all document-related routes for the AcademicRAG API.
"""
...
async def pipeline_enqueue_file(rag: AcademicRAG, file_path: Path) -> bool:
    """Add a file to the queue for processing

    Args:
        rag: AcademicRAG instance
...
async def pipeline_index_file(rag: AcademicRAG, file_path: Path):
    """Index a file
...
async def pipeline_index_files(rag: AcademicRAG, file_paths: List[Path]):
...
async def pipeline_index_texts(rag: AcademicRAG, texts: List[str]):
...
async def run_scanning_process(rag: AcademicRAG, doc_manager: DocumentManager):
...
def create_document_routes(
    rag: AcademicRAG, doc_manager: DocumentManager, api_key: Optional[str] = None
):
    ...
    async def get_pipeline_status() -> PipelineStatusResponse:
        ...
        """
        try:
            from academicrag.kg.shared_storage import get_namespace_data

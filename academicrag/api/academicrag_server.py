...
from academicrag.academicrag import AcademicRAG, QueryParam
from academicrag.base import DocProcessingStatus, DocStatus
from academicrag.types import GPTKeywordExtractionFormat
from academicrag.api import __api_version__
from academicrag.utils import EmbeddingFunc
from academicrag.utils import logger
...
def display_splash_screen(args: argparse.Namespace) -> None:
    """
    Display a colorful splash screen showing AcademicRAG server configuration
    ...
    ASCIIColors.cyan(f"""
    ╔══════════════════════════════════════════════════════════════╗
    ║                   🚀 AcademicRAG Server v{__api_version__}                  ║
    ...
    ASCIIColors.yellow(f"{ollama_server_infos.ACADEMICRAG_MODEL}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="AcademicRAG FastAPI Server with separate working and input directories"
    )

    parser.add_argument(
        "--kv-storage",
        default=get_env_value(
            "ACADEMICRAG_KV_STORAGE", DefaultRAGStorageConfig.KV_STORAGE
        ),
    ...
    parser.add_argument(
        "--doc-status-storage",
        default=get_env_value(
            "ACADEMICRAG_DOC_STATUS_STORAGE", DefaultRAGStorageConfig.DOC_STATUS_STORAGE
        ),
    ...
    parser.add_argument(
        "--graph-storage",
        default=get_env_value(
            "ACADEMICRAG_GRAPH_STORAGE", DefaultRAGStorageConfig.GRAPH_STORAGE
        ),
    ...
    parser.add_argument(
        "--vector-storage",
        default=get_env_value(
            "ACADEMICRAG_VECTOR_STORAGE", DefaultRAGStorageConfig.VECTOR_STORAGE
        ),
    ...
    parser.add_argument(
        "--key",
        type=str,
        default=get_env_value("ACADEMICRAG_API_KEY", None),
        help="API key for authentication. This protects academicrag server against unauthorized access",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=get_env_value(
            "SIMULATED_MODEL_NAME", ollama_server_infos.ACADEMICRAG_MODEL
        ),
        help="Number of conversation history turns to include (default: from env or 3)",
    )
    ...
    ollama_server_infos.ACADEMICRAG_MODEL = args.simulated_model_name
...
def create_app(args):
    ...
    from academicrag.utils import set_verbose_debug
    ...
    api_key = os.getenv("ACADEMICRAG_API_KEY") or args.key
    ...
    # Initialize FastAPI
    app = FastAPI(
        title="AcademicRAG API",
        description="API for querying text using AcademicRAG with separate storage and input directories"
        + "(With authentication)"
        if api_key
        else "",
    ...
    if args.llm_binding == "lollms" or args.embedding_binding == "lollms":
        from academicrag.llm.lollms import lollms_model_complete, lollms_embed
    ...
    if args.llm_binding == "azure_openai" or args.embedding_binding == "azure_openai":
        from academicrag.llm.azure_openai import (
            azure_openai_complete_if_cache,
            azure_openai_embed,
        )
    if args.llm_binding_host == "openai-ollama" or args.embedding_binding == "ollama":
        from academicrag.llm.openai import openai_complete_if_cache
        from academicrag.llm.ollama import ollama_embed
    ...
    async def clear_documents():
        """
        Clear all documents from the AcademicRAG system.

        This endpoint deletes all text chunks, entities vector database, and relationships vector database,
        effectively clearing all documents from the AcademicRAG system.

...
from academicrag.utils import encode_string_by_tiktoken
...
# query mode according to query prefix (bypass is not AcademicRAG quer mode)
...
class OllamaAPI:
    def __init__(self, rag: AcademicRAG, top_k: int = 60):
        self.rag = rag
    ...
    def setup_routes(self):
        ...
        async def get_tags():
            ...
            return OllamaTagResponse(
                models=[
                    {
                        "name": self.ollama_server_infos.ACADEMICRAG_MODEL,
                        "model": self.ollama_server_infos.ACADEMICRAG_MODEL,
                        "size": self.ollama_server_infos.ACADEMICRAG_SIZE,
                        "digest": self.ollama_server_infos.ACADEMICRAG_DIGEST,
                        "modified_at": self.ollama_server_infos.ACADEMICRAG_CREATED_AT,
                        "details": {
                            "parent_model": "",
                            "format": "gguf",
                            "family": self.ollama_server_infos.ACADEMICRAG_NAME,
                            "families": [self.ollama_server_infos.ACADEMICRAG_NAME],
                            "parameter_size": "13B",
        ...
        async def generate(raw_request: Request, request: OllamaGenerateRequest):
            ...
            For compatibility purpose, the request is not processed by AcademicRAG,
                    ...
                    async def stream_generator():
                        try:
                            ...
                            if isinstance(response, str):
                                ...
                                data = {
                                    "model": self.ollama_server_infos.ACADEMICRAG_MODEL,
                                    "created_at": self.ollama_server_infos.ACADEMICRAG_CREATED_AT,
                                    "response": response,
                                    "done": False,
                                }
        ...
        async def chat(raw_request: Request, request: OllamaChatRequest):
            ...
            Routes user queries through AcademicRAG by selecting query mode based on prefix indicators.

...
import os
import logging
from academicrag.kg.shared_storage import finalize_share_data
...
errorlog = os.getenv("ERROR_LOG", log_file_path)  # Default write to academicrag.log
accesslog = os.getenv("ACCESS_LOG", log_file_path)  # Default write to academicrag.log
...
def on_starting(server):
    ...
    # Log the location of the AcademicRAG log file
    print(f"AcademicRAG log file: {log_file_path}\n")
...
def post_fork(server, worker):
    ...
    setup_logger("academicrag", log_level, add_filter=True, log_file_path=log_file_path)

    # Set up academicrag submodule loggers
    for name in logging.root.manager.loggerDict:
        if name.startswith("academicrag."):

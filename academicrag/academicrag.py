...
@dataclass
class AcademicRAG:
    """AcademicRAG: Simple and Fast Retrieval-Augmented Generation."""

    # Directory
    # ---

    working_dir: str = field(
        default=f"./academicrag_cache_{datetime.now().strftime('%Y-%m-%d-%H:%M:%S')}"
    )
    ...
    """If True, academicrag will automatically calls initialize_storages and finalize_storages at the appropriate times."""
    ...
    def __post_init__(self):
        from academicrag.kg.shared_storage import (
        ...
        logger.debug(f"AcademicRAG init with param:\n  {_print_config}\n")
        ...
        self.keywords_vdb: BaseVectorStorage = self.vector_db_storage_cls(  # type: ignore
            ...
            embedding_func=self.embedding_func,
            meta_fields={"keywords", "source_id","file_path"},
        )
    ...
    async def apipeline_process_enqueue_documents(
        ...
        async with pipeline_status_lock:
            ...
            if not pipeline_status.get("busy", False):
                ...
                self.ensure_history_messages_initialized(pipeline_status)
    ...
    async def aquery(
        ...
        Returns:
            str: The result of the query execution.
        """
        if param.mode in ["subgraph", "global", "hybrid"]:
    ...
    async def adelete_by_doc_id(self, doc_id: str) -> None:
        ...
        try:
            ...
            # 3. Before deleting, check the related entities, relationships and keywords for these chunks
            for chunk_id in chunk_ids:
                ...
                # Check keywords
                keywords_storage = await self.keywords_vdb.client_storage
                keywords = [
                    dp
                    for dp in keywords_storage["data"]
                    if chunk_id in dp.get("source_id")
                ]
            ...
            keywords_to_update = {}  # keyword -> new_source_id
            ...
            # Process keywords
            all_keywords= await self.keywords_vdb.search_by_prefix(prefix="key-")
            for keyword in all_keywords:
                src_id=keyword.get("source_id")
                if src_id:
                    sources = set(src_id.split(GRAPH_FIELD_SEP))
                    sources.difference_update(chunk_ids)
                    if not sources:
                        keywords_to_delete.add(keyword["__id__"])
                        logger.debug(
                            f"Keyword {keyword['keywords']} marked for deletion - no remaining sources"
                        )
                    else:
                        new_source_id = GRAPH_FIELD_SEP.join(sources)
                        keywords_to_update[keyword["keywords"]] = new_source_id
                        logger.debug(
                            f"Keyword {keyword['keywords']} will be updated with new source_id: {new_source_id}"
                        )
            ...
            if keywords_to_delete:
                for keyword in keywords_to_delete:
                    keyword_id=compute_mdhash_id(keyword, prefix="key-")
                    await self.keywords_vdb.delete([keyword_id])
                    logger.debug(f"Deleted keyword {keyword} from vector DB")
                logger.debug(
                    f"Deleted {len(keywords_to_delete)} keywords from graph"
                )

            # Update keywords
            for keyword, new_source_id in keywords_to_update.items():
                keyword_id=compute_mdhash_id(keyword, prefix="key-")
                keyword_data = await self.keywords_vdb.get_by_id(keyword_id)
                if keyword_data:
                    keyword_data["source_id"] = new_source_id
                    await self.keywords_vdb.upsert(keyword_data)
                    logger.debug(
                        f"Updated keyword {keyword} with new source_id: {new_source_id}"
                    )
            ...
            logger.info(
                ...
                f"Deleted {len(entities_to_delete)} entities, {len(relationships_to_delete)} relationships and {len(keywords_to_delete)} keywords. "
                f"Updated {len(entities_to_update)} entities, {len(relationships_to_update)} relationships and {len(keywords_to_update)} keywords."
            )
    ...
    async def aclear_cache(self, modes: list[str] | None = None) -> None:
        ...
        Args:
            modes (list[str] | None): Modes of cache to clear. Options: ["default", "naive", "subgraph", "global", "hybrid", "mix"].
        ...
        valid_modes = ["default", "naive", "subgraph", "global", "hybrid", "mix"]
    ...
    def ensure_history_messages_initialized(self,pipeline_status):
    ...
    async def aexport_data(
        ...
        if file_format == "csv":
            ...
            with open(output_path, "w", encoding="utf-8") as mdfile:
                mdfile.write("# AcademicRAG Data Export\n\n")
            ...
            with open(output_path, "w", encoding="utf-8") as txtfile:
                txtfile.write("ACADEMICRAG DATA EXPORT\n")
                txtfile.write("=" * 80 + "\n\n")

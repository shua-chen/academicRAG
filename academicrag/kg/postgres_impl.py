...
class PGDocStatusStorage(DocStatusStorage):
    ...
    async def get_by_id(self, id: str) -> Union[dict[str, Any], None]:
        sql = "select * from ACADEMICRAG_DOC_STATUS where workspace=$1 and id=$2"
        params = {"workspace": self.db.workspace, "id": id}
    ...
    async def get_docs_by_status(
        ...
        sql = "select * from ACADEMICRAG_DOC_STATUS where workspace=$1 and status=$2"
    ...
    async def upsert(self, data: dict[str, dict[str, Any]]) -> None:
        ...
        sql = """insert into ACADEMICRAG_DOC_STATUS(workspace,id,content,content_summary,content_length,chunks_count,status)
...
class PGGraphStorage(BaseGraphStorage):
    def __post_init__(self):
        self.graph_name = self.namespace or os.environ.get("AGE_GRAPH_NAME", "academicrag")
...
NAMESPACE_TABLE_MAP = {
    ...
    NameSpace.VECTOR_STORE_ENTITIES: "ACADEMICRAG_VDB_ENTITY",
    NameSpace.VECTOR_STORE_RELATIONSHIPS: "ACADEMICRAG_VDB_RELATION",
    NameSpace.DOC_STATUS: "ACADEMICRAG_DOC_STATUS",
    NameSpace.KV_STORE_LLM_RESPONSE_CACHE: "ACADEMICRAG_LLM_CACHE",
}
...
TABLES = {
    "ACADEMICRAG_DOC_FULL": {
        "ddl": """CREATE TABLE ACADEMICRAG_DOC_FULL (
	                ...
	                CONSTRAINT ACADEMICRAG_DOC_FULL_PK PRIMARY KEY (workspace, id)
                    )"""
    },
    "ACADEMICRAG_DOC_CHUNKS": {
        "ddl": """CREATE TABLE ACADEMICRAG_DOC_CHUNKS (
                    ...
                    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    update_time TIMESTAMP,
	                CONSTRAINT ACADEMICRAG_DOC_CHUNKS_PK PRIMARY KEY (workspace, id)
                    )"""
    },
    "ACADEMICRAG_VDB_ENTITY": {
        "ddl": """CREATE TABLE ACADEMICRAG_VDB_ENTITY (
                    id VARCHAR(255),
                    workspace VARCHAR(255),
                    ...
                    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    update_time TIMESTAMP,
	                CONSTRAINT ACADEMICRAG_VDB_ENTITY_PK PRIMARY KEY (workspace, id)
                    )"""
    },
    "ACADEMICRAG_VDB_RELATION": {
        "ddl": """CREATE TABLE ACADEMICRAG_VDB_RELATION (
                    id VARCHAR(255),
                    workspace VARCHAR(255),
                    ...
                    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    update_time TIMESTAMP,
	                CONSTRAINT ACADEMICRAG_VDB_RELATION_PK PRIMARY KEY (workspace, id)
                    )"""
    },
    "ACADEMICRAG_LLM_CACHE": {
        "ddl": """CREATE TABLE ACADEMICRAG_LLM_CACHE (
                    ...
                    create_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    update_time TIMESTAMP,
	                CONSTRAINT ACADEMICRAG_LLM_CACHE_PK PRIMARY KEY (workspace, mode, id)
                    )"""
    },
    "ACADEMICRAG_DOC_STATUS": {
        "ddl": """CREATE TABLE ACADEMICRAG_DOC_STATUS (
	               workspace varchar(255) NOT NULL,
	               id varchar(255) NOT NULL,
	               content TEXT NULL,
	               ...
	               CONSTRAINT ACADEMICRAG_DOC_STATUS_PK PRIMARY KEY (workspace, id)
	              )"""
    },
}
...
SQL_TEMPLATES = {
    ...
    "get_by_id_full_docs": """SELECT id, COALESCE(content, '') as content
                                FROM ACADEMICRAG_DOC_FULL WHERE workspace=$1 AND id=$2
                            """,
    "get_by_id_text_chunks": """SELECT id, tokens, COALESCE(content, '') as content,
                                chunk_order_index, full_doc_id
                                FROM ACADEMICRAG_DOC_CHUNKS WHERE workspace=$1 AND id=$2
                            """,
    "get_by_id_llm_response_cache": """SELECT id, original_prompt, COALESCE(return_value, '') as "return", mode
                                FROM ACADEMICRAG_LLM_CACHE WHERE workspace=$1 AND mode=$2
                               """,
    "get_by_mode_id_llm_response_cache": """SELECT id, original_prompt, COALESCE(return_value, '') as "return", mode
                           FROM ACADEMICRAG_LLM_CACHE WHERE workspace=$1 AND mode=$2 AND id=$3
                          """,
    "get_by_ids_full_docs": """SELECT id, COALESCE(content, '') as content
                                 FROM ACADEMICRAG_DOC_FULL WHERE workspace=$1 AND id IN ({ids})
                            """,
    "get_by_ids_text_chunks": """SELECT id, tokens, COALESCE(content, '') as content,
                                  chunk_order_index, full_doc_id
                                   FROM ACADEMICRAG_DOC_CHUNKS WHERE workspace=$1 AND id IN ({ids})
                                """,
    "get_by_ids_llm_response_cache": """SELECT id, original_prompt, COALESCE(return_value, '') as "return", mode
                                 FROM ACADEMICRAG_LLM_CACHE WHERE workspace=$1 AND mode= IN ({ids})
                                """,
    "filter_keys": "SELECT id FROM {table_name} WHERE workspace=$1 AND id IN ({ids})",
    "upsert_doc_full": """INSERT INTO ACADEMICRAG_DOC_FULL (id, content, workspace)
                        VALUES ($1, $2, $3)
                        ON CONFLICT (workspace,id) DO UPDATE
                           SET content = $2, update_time = CURRENT_TIMESTAMP
                       """,
    "upsert_llm_response_cache": """INSERT INTO ACADEMICRAG_LLM_CACHE(workspace,id,original_prompt,return_value,mode)
                                      VALUES ($1, $2, $3, $4, $5)
                                     ...
                                     """,
    "upsert_chunk": """INSERT INTO ACADEMICRAG_DOC_CHUNKS (workspace, id, tokens,
                      ...
                      update_time = CURRENT_TIMESTAMP
                     """,
    "upsert_entity": """INSERT INTO ACADEMICRAG_VDB_ENTITY (workspace, id, entity_name, content, content_vector)
                      VALUES ($1, $2, $3, $4, $5)
                      ON CONFLICT (workspace,id) DO UPDATE
                      ...
                      update_time=CURRENT_TIMESTAMP
                     """,
    "upsert_relationship": """INSERT INTO ACADEMICRAG_VDB_RELATION (workspace, id, source_id,
    ...
    "entities": """SELECT entity_name FROM
        ...
        FROM ACADEMICRAG_VDB_ENTITY where workspace=$1)
    ...
    "relationships": """SELECT source_id as src_id, target_id as tgt_id FROM
        ...
        FROM ACADEMICRAG_VDB_RELATION where workspace=$1)
        WHERE distance>$2 ORDER BY distance DESC  LIMIT $3
       """,
    "chunks": """SELECT id FROM
        (SELECT id, 1 - (content_vector <=> '[{embedding_string}]'::vector) as distance
        FROM ACADEMICRAG_DOC_CHUNKS where workspace=$1)
        WHERE distance>$2 ORDER BY distance DESC  LIMIT $3
       """,
    # DROP tables
    "drop_all": """
	    DROP TABLE IF EXISTS ACADEMICRAG_DOC_FULL CASCADE;
	    DROP TABLE IF EXISTS ACADEMICRAG_DOC_CHUNKS CASCADE;
	    DROP TABLE IF EXISTS ACADEMICRAG_LLM_CACHE CASCADE;
	    DROP TABLE IF EXISTS ACADEMICRAG_VDB_ENTITY CASCADE;
	    DROP TABLE IF EXISTS ACADEMICRAG_VDB_RELATION CASCADE;
       """,
    "drop_doc_full": """
	    DROP TABLE IF EXISTS ACADEMICRAG_DOC_FULL CASCADE;
       """,

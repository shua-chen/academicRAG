...
from academicrag.types import KnowledgeGraph
...
class OracleDB:
    ...
    async def check_tables(self):
        for k, v in TABLES.items():
            try:
                if k.lower() == "academicrag_graph":
                    await self.query(
                        "SELECT id FROM GRAPH_TABLE (academicrag_graph MATCH (a) COLUMNS (a.id)) fetch first row only"
                    )
                else:
...
N_T = {
    NameSpace.KV_STORE_FULL_DOCS: "ACADEMICRAG_DOC_FULL",
    NameSpace.KV_STORE_TEXT_CHUNKS: "ACADEMICRAG_DOC_CHUNKS",
    NameSpace.VECTOR_STORE_CHUNKS: "ACADEMICRAG_DOC_CHUNKS",
    NameSpace.VECTOR_STORE_ENTITIES: "ACADEMICRAG_GRAPH_NODES",
    NameSpace.VECTOR_STORE_RELATIONSHIPS: "ACADEMICRAG_GRAPH_EDGES",
}
...
TABLES = {
    "ACADEMICRAG_DOC_FULL": {
        "ddl": """CREATE TABLE ACADEMICRAG_DOC_FULL (
                    id varchar(256),
    ...
    "ACADEMICRAG_DOC_CHUNKS": {
        "ddl": """CREATE TABLE ACADEMICRAG_DOC_CHUNKS (
                    id varchar(256),
                    workspace varchar(1024),
    ...
    "ACADEMICRAG_GRAPH_NODES": {
        "ddl": """CREATE TABLE ACADEMICRAG_GRAPH_NODES (
    ...
    "ACADEMICRAG_GRAPH_EDGES": {
        "ddl": """CREATE TABLE ACADEMICRAG_GRAPH_EDGES (
                    id NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                    workspace varchar(1024),
    ...
    "ACADEMICRAG_LLM_CACHE": {
        "ddl": """CREATE TABLE ACADEMICRAG_LLM_CACHE (
    ...
    "ACADEMICRAG_GRAPH": {
        "ddl": """CREATE OR REPLACE PROPERTY GRAPH academicrag_graph
                VERTEX TABLES (
                    academicrag_graph_nodes KEY (id)
                        LABEL entity
                        PROPERTIES (id,workspace,name) -- ,entity_type,description,source_chunk_id)
                )
                EDGE TABLES (
                    academicrag_graph_edges KEY (id)
                        SOURCE KEY (source_name) REFERENCES academicrag_graph_nodes(name)
                        DESTINATION KEY (target_name) REFERENCES academicrag_graph_nodes(name)
                        LABEL  has_relation
...
SQL_TEMPLATES = {
    # SQL for KVStorage
    "get_by_id_full_docs": "select ID,content,status from ACADEMICRAG_DOC_FULL where workspace=:workspace and ID=:id",
    "get_by_id_text_chunks": "select ID,TOKENS,content,CHUNK_ORDER_INDEX,FULL_DOC_ID,status from ACADEMICRAG_DOC_CHUNKS where workspace=:workspace and ID=:id",
    "get_by_id_llm_response_cache": """SELECT id, original_prompt, NVL(return_value, '') as "return", cache_mode as "mode"
        FROM ACADEMICRAG_LLM_CACHE WHERE workspace=:workspace AND id=:id""",
    "get_by_mode_id_llm_response_cache": """SELECT id, original_prompt, NVL(return_value, '') as "return", cache_mode as "mode"
        FROM ACADEMICRAG_LLM_CACHE WHERE workspace=:workspace AND cache_mode=:cache_mode AND id=:id""",
    "get_by_ids_llm_response_cache": """SELECT id, original_prompt, NVL(return_value, '') as "return", cache_mode as "mode"
        FROM ACADEMICRAG_LLM_CACHE WHERE workspace=:workspace  AND id IN ({ids})""",
    "get_by_ids_full_docs": "select t.*,createtime as created_at from ACADEMICRAG_DOC_FULL t where workspace=:workspace and ID in ({ids})",
    "get_by_ids_text_chunks": "select ID,TOKENS,content,CHUNK_ORDER_INDEX,FULL_DOC_ID  from ACADEMICRAG_DOC_CHUNKS where workspace=:workspace and ID in ({ids})",
    "get_by_status_ids_full_docs": "select id,status from ACADEMICRAG_DOC_FULL t where workspace=:workspace AND status=:status and ID in ({ids})",
    "get_by_status_ids_text_chunks": "select id,status from ACADEMICRAG_DOC_CHUNKS where workspace=:workspace and status=:status ID in ({ids})",
    "get_by_status_full_docs": "select id,status from ACADEMICRAG_DOC_FULL t where workspace=:workspace AND status=:status",
    "get_by_status_text_chunks": "select id,status from ACADEMICRAG_DOC_CHUNKS where workspace=:workspace and status=:status",
    "filter_keys": "select id from {table_name} where workspace=:workspace and id in ({ids})",
    "merge_doc_full": """MERGE INTO ACADEMICRAG_DOC_FULL a
        USING DUAL
        ON (a.id = :id and a.workspace = :workspace)
        WHEN NOT MATCHED THEN
        INSERT(id,content,workspace) values(:id,:content,:workspace)""",
    "merge_chunk": """MERGE INTO ACADEMICRAG_DOC_CHUNKS
        USING DUAL
    ...
    "upsert_llm_response_cache": """MERGE INTO ACADEMICRAG_LLM_CACHE a
        USING DUAL
        ON (a.id = :id)
        WHEN NOT MATCHED THEN
    ...
    "entities": """SELECT name as entity_name FROM
        ...
        FROM ACADEMICRAG_GRAPH_NODES WHERE workspace=:workspace)
    ...
    "relationships": """SELECT source_name as src_id, target_name as tgt_id FROM
        ...
        FROM ACADEMICRAG_GRAPH_EDGES WHERE workspace=:workspace)
        WHERE distance>:better_than_threshold ORDER BY distance ASC FETCH FIRST :top_k ROWS ONLY""",
    "chunks": """SELECT id FROM
        (SELECT id,VECTOR_DISTANCE(content_vector,vector(:embedding_string,{dimension},{dtype}),COSINE) as distance
        FROM ACADEMICRAG_DOC_CHUNKS WHERE workspace=:workspace)
        WHERE distance>:better_than_threshold ORDER BY distance ASC FETCH FIRST :top_k ROWS ONLY""",
    # SQL for GraphStorage
    "has_node": """SELECT * FROM GRAPH_TABLE (academicrag_graph
        MATCH (a)
        WHERE a.workspace=:workspace AND a.name=:node_id
        COLUMNS (a.name))""",
    "has_edge": """SELECT * FROM GRAPH_TABLE (academicrag_graph
        MATCH (a) -[e]-> (b)
    ...
    "node_degree": """SELECT count(1) as degree FROM GRAPH_TABLE (academicrag_graph
        MATCH (a)-[e]->(b)
        WHERE e.workspace=:workspace and a.workspace=:workspace and b.workspace=:workspace
    ...
    "get_node": """SELECT t1.name,t2.entity_type,t2.source_chunk_id as source_id,NVL(t2.description,'') AS description
        FROM GRAPH_TABLE (academicrag_graph
        MATCH (a)
        WHERE a.workspace=:workspace AND a.name=:node_id
        COLUMNS (a.name)
        ) t1 JOIN ACADEMICRAG_GRAPH_NODES t2 on t1.name=t2.name
    ...
    "get_edge": """SELECT t1.source_id,t2.weight,t2.source_chunk_id as source_id,t2.keywords,
        ...
        ) t1 JOIN ACADEMICRAG_GRAPH_EDGES t2 on t1.id=t2.id""",
    ...
    "merge_node": """MERGE INTO ACADEMICRAG_GRAPH_NODES a
                    USING DUAL
    ...
    "merge_edge": """MERGE INTO ACADEMICRAG_GRAPH_EDGES a
                    USING DUAL
    ...
    "get_all_nodes": """WITH t0 AS (
                        ...
                        FROM t1 LEFT JOIN academicrag_doc_chunks t2 ON t1.source_chunk_id = t2.id
                        GROUP BY t1.id
                    )
    ...
    "get_all_edges": """SELECT t1.id,t1.keywords as label,t1.keywords, t1.source_name as source, t1.target_name as target,
                ...
                FROM ACADEMICRAG_GRAPH_EDGES t1
                LEFT JOIN ACADEMICRAG_DOC_CHUNKS t2 on t1.source_chunk_id=t2.id
    ...
    "get_statistics": """select  count(distinct CASE WHEN type='node' THEN id END) as nodes_count,
                ...
                FROM (
                select 'node' as type, id FROM GRAPH_TABLE (academicrag_graph
                    MATCH (a) WHERE a.workspace=:workspace columns(a.name as id))
                UNION
                select 'edge' as type, TO_CHAR(id) id FROM GRAPH_TABLE (academicrag_graph

...
async def extract_entities(
    ...
    async def _process_extraction_result(
        ...
        return maybe_nodes, maybe_edges, maybe_keywords
    ...
    async def _process_single_content(chunk_key_dp: tuple[str, TextChunkSchema]):
        ...
        maybe_nodes, maybe_edges, maybe_keywords = await _process_extraction_result(
        ...
        for now_glean_index in range(entity_extract_max_gleaning):
            ...
            glean_nodes, glean_edges, glean_keywords = await _process_extraction_result(
            ...
            for keyword_name, keywords in glean_keywords.items():
                maybe_keywords[keyword_name].extend(keywords)
        ...
        keywords_count = len(maybe_keywords)
        log_message = f"  Chunk {processed_chunks}/{total_chunks}: extracted {entities_count} entities ,{relations_count} relationships (deduplicated) and {keywords_count} keywords."
    ...
    if not all_keywords_data:
        log_message = "Didn't extract any keywords"
        logger.info(log_message)
        if pipeline_status is not None:
            async with pipeline_status_lock:
                pipeline_status["latest_message"] = log_message
    ...
    log_message = f"Extracted {len(all_entities_data)} entities, {len(all_relationships_data)} relationships (deduplicated) and {len(all_keywords_data)} keywords."
    logger.info(log_message)
    if pipeline_status is not None:
        async with pipeline_status_lock:
            pipeline_status["latest_message"] = log_message
            pipeline_status["history_messages"].append(log_message)
    verbose_debug(
        f"New entities:{all_entities_data}, relationships:{all_relationships_data}, keywords:{all_keywords_data}"
    )
...
async def kg_query(
    ...
    if ll_keywords == [] and query_param.mode in ["subgraph", "hybrid"]:
        logger.warning(
    ...
    if hl_keywords == [] and query_param.mode in ["global", "hybrid"]:
        logger.warning(
            "high_level_keywords is empty, switching from %s mode to subgraph mode",
            query_param.mode,
        )
        query_param.mode = "subgraph"
...
async def kg_query_with_keywords(
    ...
    if not hl_keywords and not ll_keywords:
        ...
        return PROMPTS["fail_response"]
    if not ll_keywords and query_param.mode in ["subgraph", "hybrid"]:
        logger.warning("low_level_keywords is empty, switching to global mode.")
        query_param.mode = "global"
    if not hl_keywords and query_param.mode in ["global", "hybrid"]:
        logger.warning("high_level_keywords is empty, switching to subgraph mode.")
        query_param.mode = "subgraph"
